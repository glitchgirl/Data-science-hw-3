...
# The following scripts are different from the web page
#===================================
# Change '?' to 'NA'
titanic[titanic == '?'] <- 'NA' 
#=======================================
...
clean_titanic <- titanic %>%
  ...
  #Change data type of age, fare, sex and embarked
  mutate(age = as.numeric(age), fare=as.numeric(fare)) %>% 
  mutate(sex = factor(sex), embarked = factor(embarked)) %>% 
  
  #Convert to factor level for pclass and survi
